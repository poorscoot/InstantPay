...
//Sandbox URL
const httpHost="https://apis-i.redsys.es:20443/psd2/xs2a/api-entrada-xs2a/services/bancosantander";
...
module.exports = {
    ...
    async function getAuthCode(state){
        var httpMethod="get";
        var reqPath=`https://apis-i.redsys.es:20443/psd2/xs2a/api-oauth-xs2a/services/rest/bancosantander/authorize?response_type=code&client_id=${client_id}&scope=AIS%20PIS%20SVA&state=${state}&redirect_uri=http://localhost:3000/auth&code_challenge=prueba&code_challenge_method=plain`;
        const options = {
            method: httpMethod,
            headers: {
    ...
    async function getAccessToken(code){
        var httpMethod="post";
        var reqPath="https://apis-i.redsys.es:20443/psd2/xs2a/api-oauth-xs2a/services/rest/bancosantander/token";
    ...
    async function refreshAccessToken(refresh_token){
        var httpMethod="post";
        var reqPath = `https://apis-i.redsys.es:20443/psd2/xs2a/api-oauth-xs2a/services/rest/bancosantander/token?grant_type=refresh_token&client_id=${client_id}&refresh_token=${refresh_token}`;
        let options = {
            method: httpMethod,
            url: reqPath,
            httpsAgent: agent
...
//Old code, may come in handy
    /*getPreConsent:
    async function getPreConsent(access_token,uuid){
        var httpMethod="post";
        var reqPath=`/v1/consents`;
        ...
        console.log("2020-05-27")
        //Your certificate
        var certificate = "";
        var payload={"access":{"availableAccounts": "allAccounts"},"recurringIndicator":false,"validUntil":`${year}-${month}-${day}`,"frequencyPerDay":1,"combinedServiceIndicator":false};
        var payload64=Base64.encode(JSON.stringify(payload));
        var digest=crypto.createHash('sha256').update(payload64,"base64").digest("base64");
        var aux = `digest: SHA-256=${digest}\nx-request-id: ${uuid}\ntpp-redirect-uri: http://localhost:3000/auth`;
        const sign = crypto.createSign('SHA256');
        sign.write(aux);
        sign.end();
        const signature = sign.sign(clientSignKey, 'base64');
        //, passphrase: clientSignCer 
        let options = {
            method: httpMethod,
            headers: {
                'Authorization':`Bearer ${access_token}`,
        ...
        try {
            ...
            console.log('Pre consent request successful');
            return result.data;
    ...
    /*
    revokeConsent:
    async function revokeConsent(access_token,uuid,consent){
        var httpMethod="delete";
        var reqPath=`/v1/consents/${consent}`;
        //Your certificate
        var certificate = "";
        var digest="47DEQpj8HBSa+/TImW+5JCeuQeRkm5NMpJWZG3hSuFU=";
        var aux = `digest: SHA-256=${digest}\nx-request-id: ${uuid}`;
        ...
        try {
            ...
            console.log('Revoke consent request successful');
            console.log(result)
            return result;
    ...
    async function getAccountData(access_token,uuid,consent){
        var httpMethod="get";
        var reqPath="/v1/accounts?withBalance=false";
        //Your certificate
        var certificate = "";
        var digest="47DEQpj8HBSa+/TImW+5JCeuQeRkm5NMpJWZG3hSuFU=";
        var aux = `digest: SHA-256=${digest}\nx-request-id: ${uuid}`;
        ...
        try {
            ...
            console.log('Account list for consent request successful');
            console.log(result)
            console.log(result.data.accounts[0])
            console.log(result.data.accounts[1])
            console.log(result.data.accounts[2])
            console.log(result.data.accounts[3])
            console.log(result.data.accounts[4])
            return result;
    ...
    getConsent:
    async function getConsent(access_token,uuid,accs){
        var httpMethod="post";
        var reqPath=`/v1/consents`;
        //Your certificate
        var certificate = "";
        var payload={"access":{"balances":accs,"transactions":accs},"recurringIndicator":true,"validUntil":"2020-08-23","frequencyPerDay":5,"combinedServiceIndicator":true};
        var payload64=Base64.encode(JSON.stringify(payload));
        var digest=crypto.createHash('sha256').update(payload64,"base64").digest("base64");
        var aux = `digest: SHA-256=${digest}\nx-request-id: ${uuid}\ntpp-redirect-uri: http://localhost:3000/main`;
        const sign = crypto.createSign('SHA256');
